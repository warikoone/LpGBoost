...
'''
import sys
sys.path.append('/home/iasl/Neha_W/NeonWorkspace_1.3/ICONRepClassification_Py')
...
from src.com.prj.bundle.modelling.BiLSTMDimModel import BiLSTMSequenceDimModel
...
class Tier5Learning:
    ...
    def __init__(self):
        ...
        self.windowSize = 0
        self.PRETRAINED_GLOVE_EMBEDDING = {}
    ...
    def openConfigurationFile(self,jsonVariable):
        ...
        defaultPath = '/home/iasl/Neha_W/NeonWorkspace_1.3/ICONRepClassification_Py'
        ...
        if tokenMatcher:
            ...
            configFile="".join([defaultPath,"/config.json"])
    ...
    def call_LSTMConfiguration(self, x_train, x_test):
        ...
        sequentialInstance = BiLSTMSequenceDimModel
    ...
    def loadPosTagEmbedding(self, bufferSet):
        
        embedDataPath = self.openConfigurationFile("wordEmbeddingPath")
        self.PRETRAINED_GLOVE_EMBEDDING = KeyedVectors.load_word2vec_format(embedDataPath,binary=True)
        for token in bufferSet:
            bufferArray = np.array([])
            if token in self.PRETRAINED_GLOVE_EMBEDDING.vocab:
                bufferArray = self.PRETRAINED_GLOVE_EMBEDDING.word_vec(token)[0:self.EMBEDDING_DIM]
            else:
                self.assignRandomEmbedding(token)
                bufferArray = self.ASSIGNED_EMBEDDING[token]
            
            bufferArray = np.reshape(bufferArray,(1,self.EMBEDDING_DIM))
            self.CORPUS_EMBEDDING.update({token:bufferArray})
            
        return()
    
    def mapDataEmbeddings(self, bufferDict):
        
        for instanceIndex, instanceValue in bufferDict.items():
            instanceArray = np.array([])
            for token in instanceValue:
                if token in self.CORPUS_EMBEDDING:
                    instanceArray = self.populateArray(instanceArray, np.array(self.CORPUS_EMBEDDING.get(token)))
            
            self.x_instance.update({instanceIndex:instanceArray})
        return()   
    ...
    def readTrainingResourceData(self,fileDataPath):
        ...
        with open(fileDataPath, "r") as bufferFile:
            ...
            while len(currentData)!=0:
                ...
                if len(decoyList) == 3:
                    ...
                    contextFeatureList = list(map(lambda valueItem : str(valueItem), list(str(decoyList[2]).split(sep="|"))))
                    bufferInstanceList = []
                    for contextItem in contextFeatureList:
                        bufferInstanceList.extend(list(map(lambda valueItem : str(valueItem), contextItem.split(sep=","))))
                    tier1BufferSet.update(bufferInstanceList)
                    tier1BufferDict.update({index:bufferInstanceList})
                    self.y_instance.update({index:instanceKey})
                    if(index == 0):
                        self.instanceSpan = len(bufferInstanceList)
                        self.featureDimension = self.EMBEDDING_DIM
                    if(len(bufferInstanceList)<self.instanceSpan):
                        print(index)
                        sys.exit()
        ...
        self.loadPosTagEmbedding(tier1BufferSet)
        self.mapDataEmbeddings(tier1BufferDict)
...
seed(1)
set_random_seed(2)
learningInstance = Tier5Learning()
