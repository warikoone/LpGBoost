...
class Tier2Learning:
    ...
    def crossValidationSplit(self):
        ...
        for keyTerm, keyValue in y_FoldPredictDict.items():
            if keyValue == 1:
                if(self.instanceId.get(keyTerm)):
                    tier1BufferDict = self.instanceId.get(keyTerm)
    ...
    def readTrainingResourceData(self,fileDataPath, testInstanceDict):
            ...
            while len(currentData)!=0:
                ...
                if len(decoyList) == 3:
                    ...
                    if len(tier2BufferDict) > 0:
                        self.instanceId.update({index:tier2BufferDict})
                    else:
                        print("resource error in test instance")
                        sys.exit(0)
                    if(index == 0):
    ...
    def loadResource(self):
        ...
        testDataPath = self.openConfigurationFile("testInstanceFile")
        testInstanceDict = self.readTestResource(testDataPath)
        repDataPath = self.openConfigurationFile("iconRepresentationPath")
        self.readTrainingResourceData(repDataPath, testInstanceDict)
...
seed(1)
set_random_seed(2)
learningInstance = Tier2Learning()
