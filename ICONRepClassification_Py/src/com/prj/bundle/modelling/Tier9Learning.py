...
class Tier9Learning:
    ...
    def crossValidationSplit(self):
        ...
        y_FoldPredictDict = {}
        y_FoldGoldDict = {}
        training_index = list(self.y_instance.keys())
        test_index = list(self.y_TestInstance.keys())

        #print("\n train>>",training_index,"\n test>>",test_index)

        x_test = self.reshape3DArray(test_index, 1)
        y_test = list(map(lambda valueItem: int(valueItem), self.reshape2DArray(test_index, 1)))

        self.checkForTransformationOverlap(training_index)
        tier1ResultBufferDict = self.batchStratification(training_index)
        y_BatchPredictDict = {}
        for keyTerm, valueTerm in tier1ResultBufferDict.items():
            ...
            x_train = self.reshape3DArray(training_index, 2)
            y_train = list(map(lambda valueItem: int(valueItem), self.reshape2DArray(training_index, 2)))
            ...
            grid_search = GridSearchCV(xg_class, param_grid, scoring="neg_log_loss", n_jobs=-1, cv=10)
            grid_result = grid_search.fit(x_trainTransientStateScores, y_train)
            ...
            xg_class = xgb.XGBClassifier(max_depth=3, learning_rate=0.1, objective='binary:logistic', n_estimators=1000, verbosity=2)
            xg_class.fit(x_trainTransientStateScores, y_train)
            print(xg_class.feature_importances_)
            plot_importance(xg_class)
            pyplot.savefig('importance_rate.png')
        ...
        fWriteBuffer = open(outFileName,'a+')
...
seed(1)
set_random_seed(2)
learningInstance = Tier9Learning()
